...
export default async function EditProductPage({ params }: { params: { id: string } }) {
  ...
  const productResult = await sql`SELECT * FROM hoa WHERE ma_hoa = ${ma_hoa}`;
    ...
    <div className="max-w-5xl mx-auto space-y-12">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-6">
            ...
            <div>
                <h1 className="font-serif text-4xl font-bold text-text-main">Chỉnh Sửa Thông Tin</h1>
                <p className="text-text-muted italic mt-1 opacity-60">Thay đổi thông tin cho sản phẩm: <span className="text-sage font-bold">#{product.ma_hoa}</span></p>
            </div>
        </div>
      </div>
